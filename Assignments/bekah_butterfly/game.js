/**
 * Created by danielgraff1 on 11/9/15.
 */
// Global state
var
    canvas,
...
else {localStorage.setItem("localScore", "0");}
...
function Fish() {
    ...
    this.updatePlayingFish = function () {
        ...
        // Change to the score state when fish hits the ceiling
        if (this.y <= 2) {
            currentState = states.Score;
        }
...
function update() {
    ...
    if (currentState !== states.Score) {
        foregroundPosition = (foregroundPosition - 2) % 400; // Move left two px each frame. Wrap every 14px.
    }
...
function render() {
    ...
    backgroundSprite.draw(renderingContext, backgroundSprite.width, 0);

    corals.draw(renderingContext); //added in class for reset
    fish.draw(renderingContext); //added in class for reset

    // Draw fish and corals
...
function startMenu() {
    if(currentState === states.Splash) {
        ...
        renderingContext.fillRect(150, 50, 205, 250);
        renderingContext.fillStyle=backgroundSprite.color;
        renderingContext.font = "20px Arial";
        renderingContext.fillText("Bekah the Butterfly", 170, 80);
        renderingContext.font = "18px Arial";
        renderingContext.fillText("Help Bekah fly", 193, 150);
        renderingContext.fillText("Left click = up", 195, 200);
        renderingContext.fillText("Click to start", 200, 250);
    }
}
// Reset button
function reset() {
    if(currentState === states.Score) {
        renderingContext.fillStyle="grey";
        renderingContext.fillRect(100, 50, 180, 180);
        renderingContext.fillStyle=backgroundSprite.color;
        renderingContext.font = "20px Arial";
        renderingContext.fillText("Score: " + gameScore, 150, 110);
        renderingContext.fillText("High Score: " + highScore, 130, 70);
        renderingContext.fillText("Oops!", 160, 140);
        renderingContext.font = "18px Arial";
        renderingContext.fillText("Play Again?", 145, 180);
        okButtonSprite.draw(renderingContext, (width - okButtonSprite.width) / 2, 200);
    }
}
...
// intersection
        ...
        var cy2 = Math.min(Math.max(fish.y, this.y + this.height + 150), this.y + 2 * this.height + 80); // Must match gap in this.draw
// Closest difference
        var dx = fish.x - cx;
        var dy1 = fish.y - cy1;
        var dy2 = fish.y - cy2;
...
// Determine intersection
    ...
    this.draw = function () {
        ...
        topCoralSprite.draw(renderingContext, this.x, this.y + 150 + this.height); // Determines the gap between top and bottom corals
    };
}
...
function keepScore() {
    ...
    if(gameScore > highScore) {
        localStorage.localScore = gameScore;
        highScore = gameScore;
    }
}
